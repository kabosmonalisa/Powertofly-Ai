...
- **Logged-in nav** — add `data-auth="in"` to the `<div data-ptf-nav>` placeholder and `renderNav` swaps "Log in"+"Get matched" for the account menu: an `.avatar` + name `.btn` trigger opening a `.login-fly` of `.fly-item` rows (profile, dashboard, applications, saved, settings, log out). Optional `data-user` / `data-user-full` / `data-initials` override the sample identity. No new dropdown classes — reuses the login flyout.
- **Avatar** — `.avatar` (initials chip, solid brand green, `--r-pill`) + `.avatar-lg` modifier. Reusable anywhere; never a gradient behind initials.
